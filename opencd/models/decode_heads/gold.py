"""GOLD 解码头（基于 Changer 完整复制）。

该模块复制自 `changer.py`，将解码头类型由 `Changer` 改为 `GOLD`，并完成注册，
以便在配置中通过 `decode_head=dict(type='GOLD', ...)` 使用，和原有 Changer 并存。
"""
...
class MixFFN(BaseModule):
    """An implementation of MixFFN of Segformer. \
        Here MixFFN is used as projection head of GOLD.
    Args:
...
class GOLD(BaseDecodeHead):
    """The Head of GOLD.

    This head is copied from Changer.
